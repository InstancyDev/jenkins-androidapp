...
public class Event_fragment_DynamicBottomButtons extends Fragment implements SwipeRefreshLayout.OnRefreshListener, AdapterView.OnItemClickListener, BillingProcessor.IBillingHandler, RadioGroup.OnCheckedChangeListener, CompactCalendarView.CompactCalendarViewListener {

    String TAG = Event_fragment_DynamicBottomButtons.class.getSimpleName();
    ...
    public void getMobileCatalogObjectsData(Boolean isRefreshed) {
        ...
        if (!TABBALUE.equalsIgnoreCase("calendar")) {
            ...
            try {
                ...
                if (TABBALUE.equalsIgnoreCase("myevents")) {
                    additinalParameters = "EventComponentID=153~FilterContentType=70~eventtype=" + TABBALUE + "~HideCompleteStatus=true" + "~EnableMyEvents=true";
                } else {
                    additinalParameters = "EventComponentID=153~FilterContentType=70~eventtype=" + TABBALUE + "~HideCompleteStatus=true";
                }
...
//                parameters.put("AdditionalParams", "EventComponentID=153~FilterContentType=70~eventtype=" + TABBALUE + "~HideCompleteStatus=true");
                parameters.put("AdditionalParams", additinalParameters);
            ...
            try {
                ...
                parameters.put("ComponentID", sideMenusModel.getComponentId());
                parameters.put("ComponentInsID", sideMenusModel.getRepositoryId());
                parameters.put("AdditionalParams", "EventComponentID=" + sideMenusModel.getComponentId() + "~FilterContentType=70~eventtype=calendarview~HideCompleteStatus=true");
                parameters.put("SelectedTab", "");
                parameters.put("AddtionalFilter", "");
                parameters.put("LocationFilter", "");
                parameters.put("UserID", appUserModel.getUserIDValue());
            ...
            public void notifySuccess(String requestType, JSONObject response) {
                ...
                if (requestType.equalsIgnoreCase("CATALOGDATA")) {
                    if (response != null) {
                        ...
                        } else if (TABBALUE.equalsIgnoreCase("myevents")) {
                            StaticValues.MYEVENTS = 1;
                        }
                        try {
            ...
            public void notifySuccess(String requestType, String response) {
                ...
                if (requestType.equalsIgnoreCase("GetpeopleListingTab")) {
                    ...
                    if (response != null && response.length() > 0) {
                        ...
                        if (jsonArray != null) {
                            Log.d(TAG, "notifySuccess: " + jsonArray.length());
                            dynamicBottomButtons(returneventTabsModelList(jsonArray));
                        }
                    }
    ...
    public List<EventTabsModel> returneventTabsModelList(JSONArray jsonArray) {
        List<EventTabsModel> eventTabsModelList = new ArrayList<>();

        for (int i = 0; i < jsonArray.length(); i++) {
            JSONObject jsonObject = jsonArray.optJSONObject(i);
            EventTabsModel tabsModel = new EventTabsModel();
            tabsModel.tabID = jsonObject.optString("TabID");
            tabsModel.displayName = jsonObject.optString("DisplayName");
            tabsModel.mobileDisplayName = jsonObject.optString("MobileDisplayName");
            tabsModel.displayIcon = jsonObject.optString("DisplayIcon");
            eventTabsModelList.add(tabsModel);
        }
        return eventTabsModelList;
    }

    public void dynamicBottomButtons(final List<EventTabsModel> eventTabsModelList) {

        if (eventTabsModelList != null && eventTabsModelList.size() > 0) {
            segmentedSwitch.setWeightSum(eventTabsModelList.size());
            for (int i = 0; i < eventTabsModelList.size(); i++) {
                RadioButton rbn = new RadioButton(context);
                rbn.setId(i);
                rbn.setText(eventTabsModelList.get(i).mobileDisplayName);

                if (eventTabsModelList.size() > 3)
                    rbn.setTextSize(14.0f);
                else
                    rbn.setTextSize(16.0f);

                rbn.setPadding(1, 2, 4, 1);
                rbn.setButtonTintList(ColorStateList.valueOf(Color.parseColor(uiSettingsModel.getAppButtonBgColor())));
                rbn.setTextColor(Color.parseColor(uiSettingsModel.getAppButtonTextColor()));
                rbn.setBackgroundColor(Color.parseColor(uiSettingsModel.getAppButtonBgColor()));
                ...
                if (i == 0) {
                    rbn.setChecked(true);
                    // segmentedSwitch.performClick();
                    rbn.setTypeface(null, Typeface.BOLD);
                }

                segmentedSwitch.addView(rbn);

            }
            if (eventTabsModelList != null && eventTabsModelList.size() > 0) {
                switchCaseMethod(segmentedSwitch, 0, eventTabsModelList);
            }

            segmentedSwitch.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
                @Override
                public void onCheckedChanged(RadioGroup radioGroup, int checkedId) {
                    switchCaseMethod(radioGroup, checkedId, eventTabsModelList);
                }
            });
        }
    ...
    public void switchCaseMethod(RadioGroup radioGroup, int checkedId, List<EventTabsModel> eventTabsModelList) {

        for (int i = 0; i < radioGroup.getChildCount(); i++) {
            if (radioGroup.getChildAt(i).getId() == checkedId) {
                RadioButton radiobtn = (RadioButton) radioGroup.findViewById(checkedId);
                radiobtn.setTypeface(null, Typeface.BOLD);
                switch (eventTabsModelList.get(checkedId).tabID) {
                    ...
                    case "Additional-Program-Details":
                        ...
                        compactCalendarView.setVisibility(View.GONE);
                        YearTitle.setVisibility(View.GONE);
                        myLearninglistView.setVisibility(View.INVISIBLE);
                        nodata_Label.setText(getLocalizationValue(JsonLocalekeys.catalog_alertsubtitle_noitemstodisplay));
                        break;
                ...
                RadioButton radiobtn = (RadioButton) radioGroup.findViewById(radioGroup.getChildAt(i).getId());
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.event_fragment_new, container, false);
...
//            setCursorColor(txtSearch,Color.parseColor(uiSettingsModel.getAppLoginTextolor())); uncomment for cursor
        ...
        if (item_filter != null) {
            ...
            item_filter.setTitle(getLocalizationValue(JsonLocalekeys.advancefilter_header_filtertitlelabel));
                    ...
                    if (myLearningDetalData.isBadCancellationEnabled()) {
                        ...
                        builder.setMessage(getLocalizationValue(JsonLocalekeys.mylearning_alertsubtitle_doyouwanttocancelenrolledevent))
                            ...
                            public void onClick(DialogInterface dialog, int id) {
                                //do things
                                dialog.dismiss();
                                cancelEnrollment(myLearningDetalData, false);
                        ...
                        if (response.contains("true")) {
                            ...
                            builder.setMessage(getLocalizationValue(JsonLocalekeys.mylearning_enrollcancelalersubttitle_alertsubtitle))
                                ...
                                public void onClick(DialogInterface dialog, int id) {
                                    ...
                                    cancelEnrollment(eventModel, true);
                            ...
                            cancelEnrollment(eventModel, false);
                        }
